...
if __name__ == '__main__':
    dd = '/l/users/roba.majzoub/plip_data/PanNuke'
    
    print("Load npy files ...")
    fold1_imgs = np.load(opj(dd, 'Fold 1', 'images', 'fold1', 'images.npy')).astype(np.uint8)
    fold1_msks = np.load(opj(dd, 'Fold 1', 'masks', 'fold1', 'masks.npy')).astype(np.uint8)
    fold1_typs = np.load(opj(dd, 'Fold 1', 'images', 'fold1', 'types.npy'))
    print("Loaded Fold1")
    fold2_imgs = np.load(opj(dd, 'Fold 2', 'images', 'fold2', 'images.npy')).astype(np.uint8)
    fold2_msks = np.load(opj(dd, 'Fold 2', 'masks', 'fold2', 'masks.npy')).astype(np.uint8)
    fold2_typs = np.load(opj(dd, 'Fold 2', 'images', 'fold2', 'types.npy'))
    print("Loaded Fold2")
    
    fold3_imgs = np.load(opj(dd, 'Fold 3', 'images', 'fold3', 'images.npy')).astype(np.uint8)
    fold3_msks = np.load(opj(dd, 'Fold 3', 'masks', 'fold3', 'masks.npy')).astype(np.uint8)
    fold3_typs = np.load(opj(dd, 'Fold 3', 'images', 'fold3', 'types.npy'))
    print("Loaded Fold3")
