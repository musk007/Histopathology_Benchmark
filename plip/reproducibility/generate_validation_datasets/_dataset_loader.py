...
def process_Kather_csv(root_dir, seed=None):
    ...
    KATHER100K_CSV = opj(root_dir, "Kather_data", "image_fullpath_text_pair_100K.csv")
    KATHER7K_CSV = opj(root_dir, "Kather_data", "image_fullpath_text_pair_7K_validation.csv")

    def process_csv(path2csv, root_dir, subtype_dict):
        df = pd.read_csv(path2csv)
        df = df[["image_fullpath", "label"]]
        df.columns = ['image', 'label']
        df['image'] = [root_dir + 'Kather_data/' + v for v in df['image']]
...
def process_WSSS4LUAD_binary(root_dir, seed, train_ratio):
    ...
    path2data = opj(root_dir, 'WSSS4LUAD', '1.training')
...
def process_PanNuke(root_dir, seed=None, train_ratio=None):
    df = pd.read_csv('/l/users/roba.majzoub/plip_data/PanNuke/processed_threshold=10_0.3/PanNuke_all_binary.csv',index_col=0)
    df = df.reset_index(drop=True)
    for i in df.index:
...
def process_KIMIA_Path24(root_dir, seed=None):
    ...
    test_folder = opj(root_dir, 'Kimia_Path24C', 'kimia_path24', 'Test patches')
    data = []
