...
def convert_dataset_labels(args, df):
    ...
    if args.dataset.startswith('Kather'):
        ...
        label2digit = {'normal':0, 'benign':1, 'insitu':2, 'invasive':3}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset.startswith('BRACS'):
        label2digit = {'N':0, 'PB':1, 'UDH':2, 'FEA':3, 'ADH':4, 'DCIS':5, 'IC':6}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'BreakHist':
        label2digit = {'A':0, 'F':1, 'PT':2, 'TA':3, 'DC':4, 'LC':5, 'MC':6, 'PC':6}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'BCNB':
        label2digit = {'Invasive ductal carcinoma':0, 'Invasive lobular carcinoma':1, 'Other type':2}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'CRC_TP':
        label2digit = {'benign':0, 'complex stroma':1, 'debris':2, "inflammatory":3, "muscle":4, "stroma":5, "tumor":6}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'CAMEL':
        label2digit = {'normal':0, "tumor":1}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'LC25000_lung':
        label2digit = {'normal':0, "scc":1, "aca":2}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'LC25000_colon':
        label2digit = {'normal':0, "aca":1}
        df['label'] = df['label'].apply(lambda v: label2digit[v])

    elif args.dataset == 'SICAP':
        label2digit = {'NC':0, "G3":1, "G4":2, "G5":3}
        df['label'] = df['label'].apply(lambda v: label2digit[v])
...
def config():
    ...
    parser.add_argument("--dataset", default="Kather_train", type=str, choices=[
                                                                            'Kather_train', 
                                                                            'PanNuke', 'WSSS4LUAD_binary', 
                                                                            'DigestPath', 'BACH', 'BRACS', 
                                                                            'BreakHist', "MHIST", "BCNB",
                                                                            "CRC_TP", "CAMEL", "SICAP"]
                                                                            )
    ...
    parser.add_argument("--save_directory", default='/l/users/roba.majzoub/plip_finetune')
...
if __name__ == "__main__":
    ...
    args.save_directory = opj(args.save_directory, args.dataset)
    ...
    matching_pattern = opj(args.save_directory, args.dataset, savesubdir, 'performance_test_*.csv')
    # matching_pattern = opj(args.save_directory, args.dataset, f'train_ratio={args.percentage_of_training_data}', savesubdir, f'random_seed={args.random_seed}_*', 'performance_test_*.csv')
    ...
    for lr in lr_search_list:
        ...
        all_performance.to_csv(opj(args.save_directory, f'performance_val.csv'), index=False)
    ...
    performance_test.to_csv(opj(args.save_directory, f'performance_test_best_lr={args.learning_rate}_fold2.csv'), index=False)
